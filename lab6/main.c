...
#define KERNEL_TEMP_ADDR 0x20000000UL
#define RELOC_STUB_ADDR  0x21000000UL
#define BOOT_MAGIC 0x544F4F42UL

extern char copy_kernel_and_boot_start[];
extern char copy_kernel_and_boot_end[];

static unsigned int loaded_kernel_size = 0;
...
void load_kernel(void) {
    ...
    unsigned char *p = (unsigned char *)KERNEL_TEMP_ADDR;
    ...
    uart_puts("Kernel loaded at temporary address ");
    uart_hex(KERNEL_TEMP_ADDR);
    uart_puts("\r\n");
}

void boot_kernel(void *fdt) {
    unsigned char *src = (unsigned char *)copy_kernel_and_boot_start;
    unsigned char *dst = (unsigned char *)RELOC_STUB_ADDR;
    unsigned long stub_size = (unsigned long)copy_kernel_and_boot_end -
                              (unsigned long)copy_kernel_and_boot_start;

    if (loaded_kernel_size == 0) {
        uart_puts("No loaded kernel.\r\n");
        return;
    }
    ...
    void (*relocated_boot)(unsigned long, unsigned long, unsigned long, void *) =
        (void (*)(unsigned long, unsigned long, unsigned long, void *))RELOC_STUB_ADDR;

    relocated_boot(KERNEL_TEMP_ADDR, KERNEL_LOAD_ADDR, loaded_kernel_size, fdt);
