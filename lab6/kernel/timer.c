...
#define TIMER_TICK_HZ 32UL
#define TIMER_TICK_INTERVAL (TIMEBASE_FREQ / TIMER_TICK_HZ)
...
static void program_next_timer(void) {
    ...
    unsigned long tick_target;

    now_tick = read_time();
    tick_target = now_tick + TIMER_TICK_INTERVAL;

    if (next == 0) {
        target_tick = tick_target;
    } else if (next <= current_time_sec) { //如果下一個要expire的timer的expire time已經過了，那就把target_tick設為現在的tick加一，讓timer interrupt可以盡快發生
        target_tick = now_tick + 1;
    } else {
        target_tick = now_tick + (next - current_time_sec) * TIMEBASE_FREQ;
        if (target_tick > tick_target)
            target_tick = tick_target;
    }
...
int timer_usleep(unsigned int usec) {
    ...
    start = read_time();
    interval = (unsigned long)usec * (TIMEBASE_FREQ / 1000000UL);

    if (interval == 0)
        return -1;

    while (read_time() - start < interval){//還在等下一張frame 時間沒到給其他人用
        schedule();
    }
