...
static unsigned long initrd_base = 0;
...
int exec(const char* filename) {
    ...
    while (memcmp(p + sizeof(struct cpio_t), "TRAILER!!!", 10) != 0) {
        ...
        if (memcmp(name, filename, namesize) == 0) {
            void* code = alloc_page(); // allocate one page for user program
            void* stack = alloc_page(); // allocate one page for user stack

            if (code == 0 || stack == 0)
                return -1;

            kmemset_local(code, 0, 0x1000); // clear the code page, 0x1000=>4096=one page size
...
/*
static void irqstat_timer_callback(void *arg) {
    (void)arg;

    uart_puts("\r\n[irqstat] timer=");
    uart_put_dec(timer_irq_count);
    uart_puts(" external=");
    uart_put_dec(external_irq_count);
    uart_puts(" uart=");
    uart_put_dec(uart_irq_count);
    uart_puts(" last=");
    uart_put_dec(last_irq);
    uart_puts("\r\nopi-rv2> ");

    timer_add(irqstat_timer_callback, 0, 3);
}
*/
    ...
    while (1) {
        ...
        if (c == '\r' || c == '\n') {
            ...
            if (buf[0] == '\0') {
                ...
                } else if (exec(filename)) {
                    uart_puts("Failed to exec user program!\r\n");
                }
            }
            /*
            else if (str_equal(buf, "irqstat")) {
                uart_puts("timer_irq_count = ");
                uart_put_dec(timer_irq_count);
                uart_puts("\r\n");

                uart_puts("external_irq_count = ");
                uart_put_dec(external_irq_count);
                uart_puts("\r\n");

                uart_puts("uart_irq_count = ");
                uart_put_dec(uart_irq_count);
                uart_puts("\r\n");
