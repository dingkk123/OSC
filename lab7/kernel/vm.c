...
static unsigned long *ensure_pmd_table(unsigned long *pgd, unsigned long va) {
    unsigned long idx = VPN(va, 2); //#define VPN(va, level) (((va) >> (12 + 9 * (2))) & 0x1ff)
    unsigned long entry = pgd[idx];
    unsigned long *pmd;

    if ((entry & PTE_V) && !PTE_IS_LEAF(entry))
        return pa_to_table_ptr(PTE_TO_PA(entry)); //#define PTE_TO_PA(pte) (((pte) >> 10) << 12)
...
static unsigned long *ensure_pte_table(unsigned long *pgd, unsigned long va) {
    ...
    idx = VPN(va, 1); //#define VPN(va, level) (((va) >> (12 + 9 * (1))) & 0x1ff)
    entry = pmd[idx];

    if ((entry & PTE_V) && !PTE_IS_LEAF(entry)) //#define PTE_IS_LEAF(pte) ((pte) & (PTE_R | PTE_W | PTE_X))
    ...
    if (entry & PTE_V) {
        ...
        old_flags = PTE_FLAGS(entry); //#define PTE_FLAGS(pte) ((pte) & 0x3ffUL) 10bit
        for (unsigned long i = 0; i < ENTRIES_PER_TABLE; i++)
...
static void map_page_4k(unsigned long *pgd, unsigned long va, unsigned long pa, unsigned long flags) {
    unsigned long *pte = ensure_pte_table(pgd, va);

    pte[VPN(va, 0)] = MAKE_PTE(pa, flags);
}

static void map_pages_4k(unsigned long *pgd, unsigned long va, unsigned long pa, unsigned long size, unsigned long flags) {
    unsigned long start;
...
int vm_get_mapping(unsigned long *pgd, unsigned long va, unsigned long *pa_out, unsigned long *flags_out) {
    unsigned long entry;
    unsigned long pa;
...
int vm_share_region_cow(unsigned long *parent_pgd, unsigned long *child_pgd, unsigned long va, unsigned long size) {
    unsigned long start;
    unsigned long end;
    unsigned long limit;
...
void setup_vm(void) {
    unsigned long trap_vector = (unsigned long)handle_exception + PAGE_OFFSET;
    ...
    for (unsigned long i = 0; i < LINEAR_MAP_GIB; i++) {
        ...
        kernel_pgd[LOW_PGD_INDEX + i] = MAKE_PTE(virt_to_phys_ptr(kernel_pmd[i]), PTE_V);
        kernel_pgd[KERNEL_PGD_INDEX + i] = MAKE_PTE(virt_to_phys_ptr(kernel_pmd[i]), PTE_V);
    ...
    asm volatile(
        ...
        "csrw stvec, %1\n"
        :
        : "r"(MAKE_SATP(kernel_pgd)), "r"(trap_vector)
        : "memory");
}
