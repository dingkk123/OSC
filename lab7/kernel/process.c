...
static void process_init_vfs_context(struct task_struct *task, struct task_struct *parent);
static int process_setup_standard_fds(struct task_struct *task);

static void release_user_address_space(unsigned long *pgd, unsigned long code_pa, unsigned long code_va, unsigned long code_size,
                                       unsigned long stack_pa, unsigned long stack_va, unsigned long stack_size, unsigned long signal_stack_pa) {
    if (pgd && code_size)
...
static void process_init_vfs_context(struct task_struct *task, struct task_struct *parent) {
    ...
    task->root_dir = parent && parent->root_dir ? parent->root_dir : vfs_get_root();
...
static int process_setup_standard_fds(struct task_struct *task) {
    ...
    root = task->root_dir ? task->root_dir : vfs_get_root();
    cwd = task->cwd ? task->cwd : root;
    ...
    for (int fd = 0; fd < 3; fd++) {
        ...
        if (vfs_open_at("/dev/uart", 0, root, cwd, &file) < 0)
            return -1;
...
static unsigned long mmap_find_free_base(struct task_struct *task, unsigned long hint, unsigned long size) {
    unsigned long start;
...
int load_user_program(struct task_struct *task, const char *path) {
    ...
    task->user_code_data = (unsigned long)data;
    task->user_code_file_size = (unsigned long)filesize;
    task->user_stack = USER_STACK_BASE;
    task->user_stack_pa = 0;
    ...
    release_user_address_space(old_pgd, old_code_pa, old_code_va,old_code_size,
                               old_stack_pa, old_stack_va, old_stack_size, old_signal_stack_pa);
...
struct task_struct *process_create(const char *path, struct task_struct *parent) {
    ...
    task->exit_status = 0;
    task->waiting_pid = -1;
    process_init_vfs_context(task, parent);
    if (process_setup_standard_fds(task) < 0) {
        uart_puts("[process] setup stdio failed\r\n");
        flags = irq_save();
        thread_remove(task);
        irq_restore(flags);
...
int sys_exec(const char *path, struct pt_regs *regs) {
    ...
    if (process_setup_standard_fds(current) < 0)
        return -1;
...
long sys_fork(struct pt_regs *regs) {
    ...
    child->exit_status = 0;
    child->waiting_pid = -1;
    process_init_vfs_context(child, parent);
    for (int i = 0; i < TASK_MAX_OPEN_FILES; i++) {
        child->fd_table[i] = parent->fd_table[i];
        if (child->fd_table[i])
            vfs_file_ref(child->fd_table[i]);
    }
    ...
    child->user_code_data = parent->user_code_data;
    child->user_code_file_size = parent->user_code_file_size;
...
long sys_open(const char *pathname, int flags) {
    ...
    if (current == 0 || pathname == 0)
        return -1;

    for (fd = 0; fd < TASK_MAX_OPEN_FILES; fd++) {
        if (current->fd_table[fd] == 0)
            break;
    }

    if (fd == TASK_MAX_OPEN_FILES)
        return -1;

    root = current->root_dir ? current->root_dir : vfs_get_root();
    cwd = current->cwd ? current->cwd : root;

    if (vfs_open_at(pathname, flags, root, cwd, &file) < 0)
        return -1;
...
long sys_close(long fd) {
    ...
    if (current == 0 || fd < 0 || fd >= TASK_MAX_OPEN_FILES)
        return -1;

    if (current->fd_table[fd] == 0)
        return -1;

    if (vfs_close(current->fd_table[fd]) < 0)
        return -1;
...
long sys_write(long fd, const void *buf, unsigned long count) {
    ...
    return vfs_write(current->fd_table[fd], buf, count);
}
...
long sys_mkdir(const char *pathname, unsigned int mode) {
    ...
    return vfs_mkdir_at(pathname, root, cwd);
}
...
long sys_mount(const char *src,
    ...
    if (current == 0 || target == 0 || filesystem == 0)
        return -1;

    root = current->root_dir ? current->root_dir : vfs_get_root();
    cwd = current->cwd ? current->cwd : root;

    return vfs_mount_at(target, filesystem, root, cwd);
}
...
long sys_chdir(const char *path) {
    ...
    if (vfs_lookup_at(path, root, cwd, &target) < 0)
        return -1;
...
long sys_lseek64(long fd, long offset, int whence) {
    ...
    return vfs_lseek64(current->fd_table[fd], offset, whence);
}
...
long sys_ioctl(long fd, unsigned long request, void *arg) {
    ...
    return vfs_ioctl(current->fd_table[fd], request, arg);
}
...
unsigned long sys_mmap(void *addr, unsigned long length, int prot, int flags) {
    ...
    if (hint != 0 && (hint & (PAGE_SIZE - 1)) == 0 && !mmap_range_invalid(hint, size) && !mmap_range_overlaps(current, hint, size)) {
        start = hint;
    } 
    else {
...
int process_handle_page_fault(struct pt_regs *regs) {
    ...
    if ((regs->sstatus & SSTATUS_SPP) && fault_addr >= USER_STACK_TOP)
        return 0;
    ...
    else if (current->user_code_size && fault_addr >= current->user_code && fault_addr < current->user_code + current->user_code_size &&
               (regs->scause == SCAUSE_INST_PAGE_FAULT || regs->scause == SCAUSE_LOAD_PAGE_FAULT)) {
        valid_fault = 1;
        map_flags = PROT_USER_RX;
        code_fault = 1;
        code_offset = page - current->user_code;
    }
    else if (current->user_stack_size && fault_addr >= current->user_stack && fault_addr < current->user_stack + current->user_stack_size &&
               (regs->scause == SCAUSE_LOAD_PAGE_FAULT || regs->scause == SCAUSE_STORE_PAGE_FAULT)) {
        valid_fault = 1;
    ...
    kmemset_local(mem, 0, PAGE_SIZE);
    if (code_fault && current->user_code_data && code_offset < current->user_code_file_size) {
        copy_size = current->user_code_file_size - code_offset;
        if (copy_size > PAGE_SIZE)
            copy_size = PAGE_SIZE;
        kmemcpy_local(mem, (void *)(current->user_code_data + code_offset), copy_size);
    }
