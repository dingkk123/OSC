...
int page_ref_inc_pa(phys_addr_t pa) {
    struct page *pg = ptr_to_page((void *)phys_to_virt(pa));
...
int page_ref_count_pa(phys_addr_t pa) {
    struct page *pg = ptr_to_page((void *)phys_to_virt(pa));
...
void free(void* ptr) {
    ...
    if (pg->alloc_type == 2) {
        ...
        if (pool_idx < 0 || pool_idx >= NUM_POOLS || pg->refcount <= 0)
            return;

        chunk_size = pools[pool_idx].chunk_size;
        page_base = (unsigned long)page_to_ptr(pg);
        if (addr < page_base || addr >= page_base + PAGE_SIZE)
            return;
        if (((addr - page_base) % chunk_size) != 0)
            return;
