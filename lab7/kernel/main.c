...
void start_kernel(void *fdt) {
    ...
    uart_init_base(phys_to_virt(new_uart));
    ...
    uart_puts("reserve framebuffer\r\n");
    video_reserve_framebuffer();                              //lab5 video frame buffer
...
lab4 part
*/
    initrd_base = initrd_start ? phys_to_virt(initrd_start) : 0;
    ...
    timer_enable_interrupt(); // to enable timer interrupt after tp and runnable threads are ready
